...
class WasteDisposalRefuseWeight(BaseModel):
    """
    WasteDisposalRefuseWeight
    """ # noqa: E501
    type: StrictStr
    activity: Optional[StrictStr] = 'refuse'
    ...
    def specification_validate_enum(cls, value):
        ...
        if value not in set(['household_residual_waste', 'organic:food_and_drink_waste', 'organic:garden_waste', 'organic:mixed_food_and_garden_waste', 'commercial_and_industrial_waste', 'average']):
            raise ValueError("must be one of enum values ('household_residual_waste', 'organic:food_and_drink_waste', 'organic:garden_waste', 'organic:mixed_food_and_garden_waste', 'commercial_and_industrial_waste', 'average')")
        return value
    ...
    def detail_validate_enum(cls, value):
        ...
        if value not in set(['combustion', 'landfill', 'average', 'composting', 'anaerobic_digestion']):
            raise ValueError("must be one of enum values ('combustion', 'landfill', 'average', 'composting', 'anaerobic_digestion')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "type": obj.get("type"),
            "activity": obj.get("activity") if obj.get("activity") is not None else 'refuse',
