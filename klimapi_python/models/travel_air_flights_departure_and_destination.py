...
class TravelAirFlightsDepartureAndDestination(BaseModel):
    """
    TravelAirFlightsDepartureAndDestination
    """ # noqa: E501
    type: StrictStr
    activity: Optional[StrictStr] = 'flights'
    ...
    __properties: ClassVar[List[str]] = ["type", "activity", "specification", "detail", "departure", "destination", "return_trip", "passengers"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['travel-air']):
            raise ValueError("must be one of enum values ('travel-air')")
        return value
    ...
    def specification_validate_enum(cls, value):
        ...
        if value not in set(['short-haul', 'long-haul', 'average']):
            raise ValueError("must be one of enum values ('short-haul', 'long-haul', 'average')")
        return value
    ...
    def detail_validate_enum(cls, value):
        ...
        if value not in set(['economy_class', 'business_class', 'average', 'premium_economy_class', 'first_class']):
            raise ValueError("must be one of enum values ('economy_class', 'business_class', 'average', 'premium_economy_class', 'first_class')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "type": obj.get("type"),
            "activity": obj.get("activity") if obj.get("activity") is not None else 'flights',
