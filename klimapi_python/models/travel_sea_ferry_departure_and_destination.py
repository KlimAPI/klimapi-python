...
class TravelSeaFerryDepartureAndDestination(BaseModel):
    """
    TravelSeaFerryDepartureAndDestination
    """ # noqa: E501
    type: StrictStr
    activity: Optional[StrictStr] = 'ferry'
    ...
    __properties: ClassVar[List[str]] = ["type", "activity", "specification", "departure", "destination", "return_trip", "passengers"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['travel-sea']):
            raise ValueError("must be one of enum values ('travel-sea')")
        return value
    ...
    def specification_validate_enum(cls, value):
        ...
        if value not in set(['foot_passenger', 'car_passenger', 'average_all_passenger', 'average']):
            raise ValueError("must be one of enum values ('foot_passenger', 'car_passenger', 'average_all_passenger', 'average')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "type": obj.get("type"),
            "activity": obj.get("activity") if obj.get("activity") is not None else 'ferry',
