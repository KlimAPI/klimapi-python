...
class MaterialUseElectricalItemsWeight(BaseModel):
    """
    MaterialUseElectricalItemsWeight
    """ # noqa: E501
    type: StrictStr
    activity: Optional[StrictStr] = 'electrical_items'
    specification: Optional[StrictStr] = 'average'
    detail: Optional[StrictStr] = 'primary_material_production'
    ...
    def specification_validate_enum(cls, value):
        ...
        if value not in set(['electrical_items-fridges_and_freezers', 'electrical_items-large', 'electrical_items-it', 'electrical_items-small', 'batteries-alkaline', 'batteries-li_ion', 'batteries-nimh', 'average']):
            raise ValueError("must be one of enum values ('electrical_items-fridges_and_freezers', 'electrical_items-large', 'electrical_items-it', 'electrical_items-small', 'batteries-alkaline', 'batteries-li_ion', 'batteries-nimh', 'average')")
        return value
    ...
    def detail_validate_enum(cls, value):
        ...
        if value not in set(['primary_material_production']):
            raise ValueError("must be one of enum values ('primary_material_production')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "type": obj.get("type"),
            "activity": obj.get("activity") if obj.get("activity") is not None else 'electrical_items',
            "specification": obj.get("specification") if obj.get("specification") is not None else 'average',
            "detail": obj.get("detail") if obj.get("detail") is not None else 'primary_material_production',
            "value": obj.get("value"),
