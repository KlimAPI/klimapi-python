...
class MaterialUseOtherWeight(BaseModel):
    """
    MaterialUseOtherWeight
    """ # noqa: E501
    type: StrictStr
    activity: Optional[StrictStr] = 'other'
    ...
    def specification_validate_enum(cls, value):
        ...
        if value not in set(['glass', 'clothing', 'food_and_drink', 'average']):
            raise ValueError("must be one of enum values ('glass', 'clothing', 'food_and_drink', 'average')")
        return value
    ...
    def detail_validate_enum(cls, value):
        ...
        if value not in set(['primary_material_production', 'closed-loop_source', 'average', 're-used']):
            raise ValueError("must be one of enum values ('primary_material_production', 'closed-loop_source', 'average', 're-used')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "type": obj.get("type"),
            "activity": obj.get("activity") if obj.get("activity") is not None else 'other',
